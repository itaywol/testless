...
fn extracts_nested_test_ids() {
    ...
    let computed = tests
        .iter()
        .find(|d| d.computed_name)
        .expect("computed-name test found");
...
fn resolves_relative_extensionless_index_and_js_suffix() {
    ...
    let l = TsLanguage;
    assert_eq!(
        l.resolve_import(Path::new("src/format.ts"), "./math", root),
        Some(PathBuf::from("src/math.ts"))
    );
    assert_eq!(
        l.resolve_import(Path::new("src/format.test.ts"), "./index", root),
        Some(PathBuf::from("src/index.ts"))
    );
    assert_eq!(
        l.resolve_import(Path::new("src/format.ts"), "./math.js", root),
        Some(PathBuf::from("src/math.ts"))
    );
    assert_eq!(
        l.resolve_import(Path::new("src/format.ts"), "vitest", root),
        None
    );
}
